 ...
 * </p>
 */
public final class StrokeDiagramView extends View implements View.OnClickListener {
    ...
    // Manual timing — replaces ValueAnimator so the OS animator duration scale cannot affect us
        ...
        if (animated) {
            if (animationStartTime >= 0) {
                if (animatingStroke < numStrokes) {
                    final float fraction = Math.min(1.0f,
                            (float)(SystemClock.elapsedRealtime() - animationStartTime) / animationDuration);
                    final float length = strokeMeasures.get(animatingStroke).getLength();
                    final DashPathEffect effect = new DashPathEffect(new float[] {length, length}, length * (1 - fraction));
                    strokeMeasures.get(animatingStroke).getPosTan(length * fraction, pos, null);
            ...
            else if (!animationPending && finishedStrokes < numStrokes) {
                animatingStroke = finishedStrokes;
                animationDuration = animationSpeed * 100L;
